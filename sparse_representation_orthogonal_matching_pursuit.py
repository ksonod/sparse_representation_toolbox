...
for k in range(1, num_iteration+1):
    ...
    print("Maximum Inner Product is found at {}-th column".format(column_idx[k-1]+1))

    As = np.zeros((A.shape[0],k))
    for j, col_idx in enumerate(np.sort(column_idx)[A.shape[1]-k:]):
        As[:, j] = A[:, col_idx]
